...
export async function getPaiaDownloadToken(
    ...
    try {
        ...
        if (!validationResult.success || !validationResult.data.success) {
            console.error("reCAPTCHA validation failed:", validationResult.error?.issues, data["error-codes"]);
        ...
        console.log(`PAIA Access: reCAPTCHA score of ${score} for action '${action}'.`);

        if (action !== expectedAction) {
            console.warn(`reCAPTCHA action mismatch. Expected: '${expectedAction}', Received: '${action}'. IP: ${ip}`);
