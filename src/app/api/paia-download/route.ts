...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        if (!blobResponse.ok || !blobResponse.body) {
            console.error(`Failed to download blob from storage. Status: ${blobResponse.status}`);
            return new NextResponse("Could not retrieve the document from storage.", {status: 502});
        }
        ...
        console.error("An unexpected error occurred while fetching the PAIA manual:", error);
